/**
 * Pulls electrician audit reports from Postgres and writes src/data/reports.json.
 *
...
function buildImpactEstimate(audit, lead) {
  ...
  const avgServiceCall = 300; // avg electrician service call value
...
async function main() {
  ...
  console.log(`Connected. Pulling all deep-audited electrician leads...`);
